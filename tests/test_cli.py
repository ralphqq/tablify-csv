...
def mocked_calls(mocker, data_and_columns, html_render_result):
    ...
    mocked_clip = mocker.patch("tablify.cli.pyperclip.copy")
...
def test_tablify_command_with_no_output_file_and_not_copied_to_clipboard(
    runner, mocked_calls
):
    mocked_csv_table, _, _, mocked_save, mocked_clip = mocked_calls
    ...
    assert not mocked_clip.called


def test_tablify_command_with_output_file_and_not_copied_to_clipboard(
    runner, mocked_calls
):
    _, _, _, mocked_save, mocked_clip = mocked_calls
...
def test_tablify_command_with_no_output_file_but_copied_to_clipboard(
    runner, mocked_calls
):
    mocked_csv_table, _, _, mocked_save, mocked_clip = mocked_calls

    result = runner.invoke(app, [INPUT_FILENAME, "--clip"])

    assert result.exit_code == 0
    mocked_csv_table.assert_called_once_with(INPUT_FILENAME)
    assert not mocked_save.called
    assert mocked_clip.called


def test_tablify_command_with_output_file_and_copied_to_clipboard(
    runner, mocked_calls
):
    _, _, _, mocked_save, mocked_clip = mocked_calls
    actual_output_file = "converted_table.html"
    ...
    result = runner.invoke(
        app, [INPUT_FILENAME, "--output-file", actual_output_file, "--clip"]
    )
...
def test_tablify_command_errors(runner, mocked_calls):
    _, _, mocked_async_render, _, _ = mocked_calls
...
def test_tablify_command_keyboard_interrupt(runner, mocked_calls, caplog):
    _, mocked_columns, _, _, _ = mocked_calls
